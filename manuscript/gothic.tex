...
Drei kleine Skizzen (siehe \autoref{fig:orpheus_inventar} oben) lassen vermuten, dass sich die Kamera -- zumindest wenn man die Spielfigur aus der Verfolgerperspektive spielt (siehe \autoref{sec:orpheus_darstellung_kamera}) -- beim Öffnen des Inventars in einem Halbkreis um die Spielfigur herum hätte bewegen sollen, so dass man sie am Ende von vorne sieht.
Diese \enquote{Steckt [ihre] Waffen weg} und wäre am Ende der Kamerafahrt außerdem nicht mehr zentriert dargestellt, sondern an den linken Bildschirmrand verschoben, so dass rechts von ihr ausreichend Platz für die Darstellung des Inventars bleibt.
...
Ein Schlüssel, ein Schwert und ein Kleidung- oder Rüstungselement für den Oberkörper, wobei das Schwert und das Kleidungs- oder Rüstungselement in zwei eigenen Kästen eingefasst sind.
Der vierte Balken scheint zweigeteilt zu sein:
In seiner oberen Hälfte werden vier Kleidung- oder Rüstungselemente dargestellt und von einer Wellenlinie umschlossen:
Ein Element für den Kopf, eines für den Oberkörper, eines für die Beine sowie eines für die Füße.
