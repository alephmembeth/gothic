...
Darüber, ob Videospiele als Kunst gelten und als legitimer Gegenstand wissenschaftlicher Untersuchung oder philosophischer Reflexion betrachtet werden können, scheint glücklicherweise immer weniger Dissens zu herrschen.\footnote{In diesem Sinne ernstgenommen werden sie etwa -- um aus der Vielzahl inzwischen verfügbarer Publikationen nur drei ausgewählte Beispiele zu nennen -- von \autocite{feige_computerspiele_2015}, \autocite{zimmermann_gameskultur_2020} sowie \autocite{beil_game_2018}.
Eine frühere Fassung dieses Kapitels ist während meiner Arbeit an diesem Buch auch als Blogbeitrag erschienen; \autocite[vgl.][]{bauer_sammeln_2025}.}
...
Sebastian Möring resümiert diesbezüglich:
...
Der Fokus scheint damit, wenig verwunderlich, auf offiziell veröffentlichtem Material zu liegen.
Nur vergleichsweise selten widmet man sich den \textit{Vorstufen} dieses Materials, den groben Ideenskizzen, den frühen Entwürfen und Designdokumenten, den Alpha- und Betaversionen der Spiele, den internen Entwicklungswerkzeugen und so weiter.
All das ist in der Regel -- anders als das fertige Spiel -- nur selten dafür bestimmt, in die Öffentlichkeit entlassen zu werden, sieht man von dem ein oder anderen Making-of-Buch oder Bildband mit Konzeptzeichnungen einmal ab.
...
Nur sporadisch finden einzelne Dokumente oder Entwicklungsversionen von Spielen ihren Weg in die Öffentlichkeit, wobei oft nur noch mit Mühe nachvollzogen werden kann, wie dieser Weg eigentlich aussah.
Durch die Weiten des Netzes geistern heute unter anderem -- um ein paar Beispiele zu nennen -- Designdokumente von \textit{Diablo} (1996), \textit{Doom} (1993), dem nie erschienenen \textit{Fallout: Brotherhood of Steel 2}, \textit{Grim Fandango} (1998), \textit{Planescape: Torment} (1999), \textit{Silent Hill} (1999) oder \textit{Tron} (1982).\footnote{Diese Beispiele hat Mikael Segedi gesammelt und zugänglich gemacht; \autocite[vgl.][]{segedi_famous_2015}.}

Manchmal sammeln Liebhaber:innen solche Dokumente online, wie beispielsweise bei \textit{Tomb Raider} (1996) oder \textit{Gothic} (2001).\autocite[Vgl.][]{core_design_tomb_raider,gothic_archive_2024}
Im Fall von \textit{Shenmue} (1999) haben sich Fans sogar zusammengeschlossen, um Material für mehrere tausend Euro bei Auktionen zu ersteigern.\footnote{\autocite[Vgl.][]{shenmue_original_2025}.
Für den Hinweis hierauf danke ich Jacob Birken.}
Manchmal wird eine solche Veröffentlichung aber auch von Entwickler:innen oder Publishern selbst forciert.
...
Andere -- inzwischen ebenso nur noch über die \textit{Wayback Machine} des \textit{Internet Archive} erreichbare -- Beispiele sind Richard Hill-Whittall, der eine Reihe von Dokumenten zu Wii- und PSVita-Spielen auf seiner Homepage veröffentlicht hatte,\autocite[Vgl.][]{hill_whittall_gdd_2017} Mike Dailly, der das Designdokument von \textit{Grand Theft Auto} (1997) auf \textit{Flickr} hochgeladen hatte,\footnote{\autocite[Vgl.][]{dailly_race_n_chase_2011}; zum zweiten Teil siehe außerdem \autocite{internet_archive_gta_2}.} Deep Silver Volition, die Dokumente des nie erschienenen \textit{Saints Row: Undercover} veröffentlicht hatten\footnote{\autocite[Vgl.][]{volition_thursday_2016}.
\enquote{Nie erschienen} trifft es dabei nicht ganz:
Im gleichen Zuge hat Volition damals auch eine spielbare Entwicklungsversion zugänglich gemacht; \autocite[vgl.][]{frank_canceled_2016}.} oder Giant Sparrow, die einen internen Trailer sowie Dokumente von \textit{What Remains of Edith Finch} (2017) auf ihrem Blog gepostet hatten.\autocite[Vgl.][]{giant_sparrow_edith_2018}
...
Gleiches gilt meiner Meinung nach übrigens auch -- aber das soll an dieser Stelle nur eine Randbemerkung bleiben -- für die Kommunikation der Entwickler:innen.\footnote{An vielen Stellen profitiert die Forschung von dem erhalten gebliebenen Schriftverkehr vergangener Generationen.
Dass ein Großteil dieses Schriftverkehrs heute elektronisch vonstattengeht und das Risiko birgt, für künftige Generationen unzugänglich zu werden, haben Errol Friedberg und Kollegen bereits Anfang des Jahrtausends in einer Zuschrift an \textit{Nature} angemahnt; \autocite[vgl.][]{friedberg_e_mail_2003}.}
...
Immerhin:
Wenn Bodo Plachta das Folgende in seiner Einführung in die Editionswissenschaft schreibt, scheint es mir vor diesem Hintergrund nicht so abwegig, das erste Wort -- \enquote{Texte} -- durch \enquote{Videospiele} zu ersetzen:
...
Ob ausgerechnet \textit{Gothic} nun in diese Reihe gehört, mag jeder für sich selbst entscheiden.
...
Wenn ich im Folgenden Archivdokumente eingehender behandle, stelle ich sie am Anfang des entsprechenden Kapitels mit einer äußerst knappen Handschriften- beziehungsweise Dokumentenbeschreibung vor.
Bei Zitaten aus solchen Dokumenten strebe ich eine zeichengetreue diplomatische Wiedergabe an.
Dementsprechend werden beispielsweise Abkürzungen nicht aufgelöst und Fehler in Orthographie oder Interpunktion bleiben erhalten.
Statt sie zu korrigieren, weise ich an den entsprechenden Stellen in eckigen Klammern mit dem lateinischen \textit{sic} (\enquote{so}, von \textit{sic erat scriptum}, \enquote{so wurde es geschrieben}) darauf hin, dass die jeweilige Stelle in dieser Form aus der Vorlage stammt und keinen Übertragungsfehler darstellt.
...
Stammen eckige Klammern jedoch aus der Vorlage selbst, weise ich in einer Fußnote darauf hin.
...
Damit hoffe ich, eine \enquote{Einebnung typographischer Besonderheiten} zu vermeiden, die sonst \enquote{auch mit einer Sinnverlagerung einhergehen} könnte.\autocite[S.~21]{plachta_editionswissenschaft_1997}
...
Auch Textfarben werden -- außer bei Erwähnung in der Handschriften- oder Dokumentenbeschreibung -- nicht ausgewiesen.
Insgesamt versuche ich, das Quellenmaterial möglichst für sich sprechen zu lassen, weswegen ich zahlreich daraus zitiere.
...
Die Kamera bewegt sich, einem roten, von Säulen gesäumten Läufer folgend, auf den König zu, der mit geneigtem Haupt, sich auf eine Lanze stützend, an der Stirnwand des Saals auf seinem schmucklosen Thron sitzt, zu dem ein paar flache Stufen hinaufführen.
Zu seiner Rechten und Linken reichen rote Banner mit goldenen Applikationen von der Decke bis zum Boden.
Der König trägt eine reich verzierte Brustplatte, um seine Schultern liegt ein Fell, die Hände stecken in schweren Panzerhandschuhen.
...
Hier scheint ein Entkommen unmöglich.
...
Die Kuppel wächst im rasanten Tempo immer weiter und umschließt bald auch den Magier in der blauen Robe, der schützend die Arme nach oben reißt, als die Barriere auf ihn zurast.
\enquote{Wir waren nun selbst Gefangene der Barriere.}

Im Innenhof der Burg stehen zwei Wachen, verwundert in den Himmel, auf die gerade entstandene blau schimmernde Kuppel, blickend.
Eine der beiden weist erstaunt mit der Hand nach oben, als sich von hinten vorsichtig ein Gefangener nähert und ihr eine Spitzhacke in den Rücken rammt.
\enquote{Mehr als eine Sekunde der Unachtsamkeit brauchten die Gefangenen nicht.}
Der Gefangene zieht seine Spitzhacke aus der bäuchlings auf dem Boden liegenden Wache, ein anderer schneidet der zweiten Wache derweil von hinten mit einem Messer die Kehle durch.
\enquote{Khorinis war nun in den Händen der Gefangenen.}

\enquote{Der König hatte keine Wahl.}
Rhobar II. erhebt sich von seinem Thron, um einem Boten eine Schriftrolle auszuhändigen.
...
\enquote{Gut}, antwortet der Gefangene mit einem schelmischen Grinsen im Gesicht, {ich werde deinen Brief nehmen.}
...
\enquote{Du wag\,$\ldots$}, hebt er an, wird aber erneut von der rot-schwarz gekleideten Gestalt unterbrochen:
\enquote{Schweig.}
Es besteht wenig Zweifel daran, wer von beiden in diesem Moment die höhere Autorität genießt.
Und so ist es der Hinzugekommene, der fortfährt:
\enquote{Gut, hinein mit ihm.}
Seine beiden Bewacher stoßen den Häftling über den Rand der Klippe, wo er durch die magische Barriere fällt. 
Das Wasser stiebt nach allen Seiten davon, als er in dem unter der Klippe gelegenen See landet.

Als er sich an das Ufer des Sees schleppt, wird er von drei rot gewandeten Gestalten empfangen.
Eine von ihnen zieht ihn zu sich herauf und begrüßt ihn mit den Worten \enquote{Willkommen im Lager} -- ehe sie ihm einen Faustschlag ins Gesicht verpasst.
...
An seinem Gürtel baumelt ein Schwert und über die Schulter hat er einen Bogen geworfen, der beinahe so groß ist, wie er selbst.
...
Dieser Machtstellung halber werden Gomez und seine Jungs auch als \enquote{Erzbarone} bezeichnet.
...
Wenn wir möchten, können wir Diego von dem Brief erzählen, den uns der der Mann in der roten Robe gegeben hat, ehe wir von der Klippe gestoßen wurden.
\enquote{Du kannst von Glück sagen, dass ich mich bei den Magiern nicht mehr blicken lassen kann}, erwidert Diego.
\enquote{Jeder andere wird dir mit Freude für diesen Brief die Kehle durchschneiden.
...
Zutritt zur Burg allerdings haben nur Gomez und seine Leute.
...
\enquote{Ja}, sagt Diego.
\enquote{Deine Begegnung mit Bullit und seinen Jungs hat dir hoffentlich gezeigt, dass du Schutz brauchst.
...
Ich bin hier, um Neuen wie dir klarzumachen, dass sie bei uns im alten Lager [sic] am besten aufgehoben sind.}
...
Auch eine Wegbeschreibung gibt uns Diego an die Hand:
...
\enquote{Aber wenn du vorhast, dich mit ihm anzulegen, sei vorsichtig}, warnt uns Diego.
\enquote{Er ist ein erfahrener Kämpfer.}
Nicht nur Bullit ist eine warnende Erwähnung wert.
\enquote{Es treiben sich jede Menge wilder Bestien zwischen den Lagern herum}, erzählt Diego.
\enquote{Ohne Waffe zu gehen ist Wahnsinn.}
Auf dem Weg zum Alten Lager sollen wir uns deswegen ein bisschen umschauen, rät er.
...
\begin{itemize}
   ...
   \item \enquote{\uline{\textsc{Der rote Faden\,$\ldots$}}},\autocite{orpheus_der_rote}
...
\noindent Daneben gibt es noch

\begin{itemize}
   \item ein Dokument zur Kampfsteuerung\autocite{orpheus_kampfsteuerung} sowie
   \item ein Dokument zu den Nicht-Spieler-Charakteren in den verschiedenen Fraktionen,\autocite{orpheus_nscs} 
\end{itemize}

\noindent die -- abgesehen von den Überschriften ihrer jeweiligen Abschnitte -- keine eigenen Titel tragen, sowie einige lose Notizen, die sich neben den oben genannten Titeln \enquote{\textsc{History}}, \enquote{\textsc{Kreaturen}}, \enquote{\textsc{Locations}}, \enquote{\uline{Übersicht}} und \enquote{\uline{Aufträge}} in einem Konvolut finden, dem ein Deckblatt mit der Aufschrift \enquote{\textsc{Orpheus. B -- Scribbles}} vorangestellt ist.\footnote{\autocite{orpheus_b_scribbles}.
Das \enquote{B} mag hier ein Nummerierungselement darstellen. % Mike Hoge fragen, worauf sich »B – Scribbles« bezieht.
\enquote{Scribbles} sind im Englischen Kritzeleien oder Schmierereien.
Damit wird aber auch, insbesondere im Medienbereich, die skizzenhafte und grobe Darstellung erster Ideen oder Entwürfe bezeichnet; \autocite[vgl.][S.~128]{seebohn_werbung_2011}.}
Einen Überblick über die einzelnen Dokumente geben \autoref{fig:orpheus_dokumente_1} und \autoref{fig:orpheus_dokumente_2} in \autoref{ch:anhang_abbildungen}.
Werfen wir im Folgenden einen kurzen Blick auf die Beschaffenheit der jeweiligen Dokumente, ehe wir uns ihrem Inhalt widmen.


% ORPHEUS – DOKUMENTE – SCRIBBLES %
Den Anfang macht das Konvolut \enquote{\textsc{Orpheus. B -- Scribbles}}.
Hierbei handelt es sich um eine Sammlung von insgesamt -- das oben erwähnte Deckblatt mitgezählt -- 19 Seiten.
Besagtes Deckblatt ist eine Blankoseite, in deren oberen Drittel der Titel mit blauem Kugelschreiber geschrieben steht.
Ihm folgen \enquote{\textsc{Orpheus -- History}} (zwei Seiten), \enquote{\textsc{Orpheus -- Kreaturen}} (eine Seite), \enquote{\textsc{Orpheus -- Locations}} (eine Seite), fünf Seiten loser Notizen, \enquote{\textsc{Orpheus} -- \uline{Übersicht}} (fünf Seiten), \enquote{\uline{Aufträge}} (zwei Seiten) sowie abschließend noch einmal zwei Seiten loser Notizen (siehe \autoref{fig:orpheus_dokumente_1}).
Auf diejenigen Elemente, die über einen eigenen Titel verfügen, gehen wir im Folgenden gesondert ein.
Widmen wir uns an dieser Stelle also erst einmal den nicht näher betitelten Notizen:
Bei sechs dieser sieben Seiten handelt es sich um Blankoseiten (aller Wahrscheinlichkeit nach Druckerpapier im Format DIN A4), eines folgt dementgegen einer karierten Lineatur und weist am linken Rand eine standardisierte Vierfachlochung sowie einen Marginalsteg auf (ganz offensichtlich einem handelsüblichen Notizblock entnommen).
Beschrieben (und bemalt) sind die sieben Seiten mit Kugelschreiber in Schwarz sowie in verschiedenen Blautönen, außerdem mit blauem Fineliner und -- an einer kleinen Stelle -- mit Bleistift.
Die Seitenorganisation ist nicht-linear und fragmentarisch; es finden sich zahlreiche Listen mit Aufzählungszeichen (Punkte, Striche, Pfeile, Nummern und Buchstaben) sowie interlineare Ergänzungen.
Verstreut -- insbesondere auf den ersten Seiten -- fallen kleinere Kritzeleien sowie -- auf den letzten beiden Seiten -- figürliche Skizzen ins Auge.


% ORPHEUS – DOKUMENTE – AUFTRAEGE %
Die \enquote{\uline{Aufträge}} sind im Konvolut \enquote{\textsc{Orpheus. B -- Scribbles}} enthalten.
Bei ihnen handelt es sich um zwei Blankoseiten, die zunächst mit einem blauen Kugelschreiber beschrieben und später mit einem schwarzen Kugenschreiber ergänzt worden sind.
Im Wesentlichen finden wir hier eine nummerierte Liste mit Ideen für Missionen sowie einen Versuch, diese Ideen zu kategorisieren (siehe \autoref{sec:orpheus_mechanik_auftraege}).


% ORPHEUS – DOKUMENTE – ROTER FADEN %
Auch die Notizen zum \enquote{\uline{\textsc{rote[n] Faden}}} sind auf Blanko-Bögen festgehalten worden.
Die erste von insgesamt vier Seiten beinhaltet eine vertikal angelegte und nummerierte Liste, verfasst mit einem ursprünglich wahrscheinlich schwarzem Stift, vermutlich einem Fineliner, wobei die Farbe mittlerweile eher ins Braune tendiert.
Die übrigen drei Seiten, mit einem schwarzen Kugelschreiber beschrieben, werden von losen Stichpunkten und Pfeildiagrammen dominiert.


% ORPHEUS – DOKUMENTE – WERDEGANG %
Beim \enquote{\uline{Werdegang eines SC's}} handelt es sich um einen beidseitig beschriebenen Blanko-Bogen.
Die Vorderseite wird fast vollständig von einer nummerierten Liste gefüllt, die in der gleichen -- inzwischen braun erscheinenden -- Farbe geschrieben wurde wie die erste Seite des \enquote{\uline{\textsc{rote[n] Faden[s]}}}.
Das Schriftbild erscheint hier vergleichsweise geordnet.
Im oberen Drittel der Rückseite finden wir ein mit blauem Fineliner erstelltes, aus zwei Ebenen bestehendes Pfeildiagramm.


% ORPHEUS – DOKUMENTE – BEWEGUNGEN %
Die \enquote{\uline{\textsc{Bewegungen}}} umfassen einen einseitig mit Bleistift beschrifteten Blank-Bogen, sowie zwei karierte Blätter -- eines davon doppelseitig beschrieben --, auf denen noch schwach ein Marginalsteg an der linken Seite zu erkennen ist.
Diese drei Seiten wiederum sind einmal mehr von dem uns inzwischen bekannten, mittlerweile ins Braune tendierenden Fineliner geprägt; von späterer Hand stammen Hinzufügungen mit Bleistift sowie in roter und blauer Farbe.
Während die erste Seite fast vollständig von einer Auflistung mit Spiegelstrichen gefüllt wird -- wobei die Notizen hier in einem großformatigen, raumgreifenden Duktus verfasst sind --, weisen die Folgeseiten eine abweichende formale Organisation auf:
Sie sind durch horizontale Trennlinien in jeweils vier bis fünf Blöcke partitioniert.
Innerhalb der jeweiligen Blöcke werden unterschiedlichen Gruppen interner Animationsbezeichnungen (bspw. \enquote{1SA00}) verschiedene Informationen zuordnet.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ORPHEUS – SPIELMECHANIK – FAEHIGKEITEN %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Während \enquote{Telekinetic Storm} erst mit dem 2015 für \textit{Pathfinder} erschienenen Quellenbuch \textit{Occult Adventures} als populärer Zauberspruch in Erscheinung zu treten scheint,\autocite[Vgl.][S.~188]{bulmahn_pathfinder_2015} kennen wir \enquote{Levitation} ebenfalls aus \textit{Advanced Dungeons \& Dragons} (dort als \enquote{Levitate} beziehungsweise \enquote{Schweben}).\footnote{Vgl. \autocite[S.~143]{cook_adnd_1989}, sowie \autocite[S.~186]{cook_adnd_1996}.} % Mike fragen, woher Telekinetic Storm stammt
...
Sie zieht sich durch einen Großteil der Notizen zu \textit{Orpheus} und kulminiert mit \enquote{\textsc{Orpheus} -- Spielwelt/Gildensystem} sowie \enquote{\textsc{Orpheus} -- Spielwelt/Gildensystem V2} in zwei Versionen eines handschriftlich verfassten und dann kopierten Formulars, in dem Mike Hoge verschiedene Eckpunkte zu den unterschiedlichen Fraktionen festgehalten hat.

Die erste Version von \enquote{\textsc{Orpheus} -- Spielwelt/Gildensystem} (siehe \autoref{fig:orpheus_gilden_1}) enthält Felder für die Orte, an denen eine Gruppe zu finden ist (\enquote{Hauptsïtz [sic]} sowie \enquote{anderweitiges Vorkommen}), für ihre \enquote{\enquote{\textsc{Soziale}} \textsc{Funktion}}, ihren \enquote{\textsc{Ausbruchsplan}} (wenn sie denn einen hat), zu welchen anderen Gruppen sie \enquote{\textsc{Gute}}, \enquote{\textsc{Neu[t]rale}} oder \enquote{\textsc{schlechte}} Beziehungen pflegt, welche Voraussetzungen man erfüllen muss, um sich der Gruppe anschließen zu können (\enquote{\textsc{Eintrittsvoraussetzungen}}), ab wann man sich der Gruppe nicht mehr anschließen kann (\enquote{\textsc{Exclusiv} [sic] \textsc{ab Level}}) und welche Fähigkeiten man dort als Mitglied erlernen kann (\enquote{\textsc{Lernprogramm}}). Diese Version kennt insgesamt 15 verschiedene Gruppierungen, die auf vier Lager aufgeteilt sind.

% ORPHEUS – SPIELWELT – FRAKTIONEN – ALTES LAGER %
...
Junta und Kampfmagier sollten im \enquote{Zentrum}\autocite[S.~1, 2]{orpheus_gildensystem} des Alten Lagers residieren, die Mafiaschläger hätte man vorrangig in einem \enquote{Kämpferviertel} und bei der \enquote{Arena}\autocite[S.~4]{orpheus_gildensystem} finden sollen, die Mafiaspitzel in einem \enquote{Diebesviertel}\autocite[S.~5]{orpheus_gildensystem} und die Alten Buddler in einem \enquote{Buddlerviertel}.\autocite[S.~3]{orpheus_gildensystem}
...
Sie sind die \enquote{Made im Speck in [dem] Zentrum des alten Lagers} und \enquote{nehmen nur SCs mit ungewöhnlichem magischen Talent} bei sich auf. \enquote{Dies \enquote{\uline{erkauft}} sich der SC bei \uline{Lehrern}}\autocite[S.~2]{orpheus_gildensystem}
...
Die Mafiaschläger ihrerseits \enquote{\uline{Verteidigen} Altes Lager, Altes Buddlerfort und Buddler bei der Arbeit}.
Sie sind außerdem die \enquote{Arenavorsitzende[n]} und \enquote{bedrohen [das] 08/15-Viertel $\rightarrow$ \enquote{Schutzgeld} für die Mafia}.
...
Ob das magische Erz zu diesem Zeitpunkt schon als ein allgemein anerkanntes Tauschmittel unter den Gefangenen konzipiert war, geht aus den Notizen nicht hervor. % Mike fragen, wann diese Idee aufkam.
