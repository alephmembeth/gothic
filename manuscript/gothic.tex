...
Im Folgenden werfen wir zuerst einen allgemeinen Blick auf diesen Fundus (\autoref{sec:orpheus_dokumente}), ehe wir uns anschauen, welche Ideen sich in Ihnen finden.
...
Die erhaltenen Dokumente lassen sich -- etwas abweichend von der Ordnung im \textit{Gothic Archive} -- anhand einiger in ihnen enthaltener Titel grob gliedern:
...
Die anderen Dokumente könnten dementsprechend etwas älter sein; der in ihnen enthaltene Entwurf einer Karte wird beispielsweise auf das Vorjahr, also 1995, geschätzt.\autocite{flosha_evolution_2024}
...
Die \enquote{\uline{\textsc{Bewegungen}}} umfassen eine mit Bleistift beschriebene Blankoseite sowie drei karierte Seiten, die größtenteils wohl mit (ursprünglich wahrscheinlich schwarzem, mittlerweile eher ins Braune tendierendem) Fineliner beschrieben sind.
Von späterer Hand stammen Hinzufügungen mit Bleistift sowie in roter und blauer Farbe.

Das \enquote{\uline{Interface}} besteht aus sieben mit (blauem) Kugelschreiber und (wiederum ursprünglich wahrscheinlich schwarzem) Fineliner beschriebenen Seiten.
...
Aus der \enquote{Zusammenfassung zum Orpheus-Konzept} schließlich ist nur ein dreiseitiger, am Computer verfasster Teil zu \enquote{\uline{Graphiken}} erhalten, der seinerseits unterteilt ist in die Abschnitte
...
Auf der letzten Seite ist das Dokument ergänzt um mit Bleistift gezeichnete Skizzen.
...
Wenn ich im Folgenden von \enquote{Spielmechanik} spreche, verstehe ich dabei relativ weit gefasst die Menge an Möglichkeiten, die Spieler:innen zur Verfügung gestellt wird, um mit Spielelementen interagieren zu können.\autocite[255]{jaervinen_games_2008}
...
Von den oben genannten Dokumenten befassen sich weite Teile von \enquote{Orpheus -- Interface}\autocite[Vgl.][]{orpheus_interface} (siehe auch \autoref{sec:orpheus_darstellung}) sowie die Notizen zur Kampfsteuerung im weitesten Sinne mit solchen spielmechanischen sowie steuerungsbezogenen Fragen.
...
Auf den ersten beiden davon, die nur noch als Schwarzweißkopie vorliegen, finden wir die Überschriften
...
Unter diesen Überschriften ist eine Reihe stichpunktartiger Ideen zu Mechanik und Steuerung des Kampfgeschehens notiert, begleitet von einer Reihe kleiner Skizzen, die in der Draufsicht Entfernungen zum Spieler und Gegnerpositionen darstellen (siehe \autoref{fig:orpheus_kampf}).
Auch auf der dritten, mit blauem Kugelschreiber beschriebenen Seite, finden wir eine solche Skizze und entsprechende Notizen.
Bei der vierten Seite handelt es sich um eine mit blauem Kugelschreiber gezeichnete und mit Bleistift ausgefüllte Tabelle, in der einige Zellen zusätzlich mit rotem oder grünem Fineliner umrandet sind.
...
\begin{figure}
   ...
   \caption{Ausschnitt aus \enquote{[Notizen zur Kampfsteuerung]}}
