...
In \enquote{\textsc{Orpheus. B -- Scribbles}} ist die Idee festgehalten, dass es sowohl \enquote{\uline{Gilden\-Lehrer}} als auch \enquote{\uline{Freie Lehrer}} gibt, die \enquote{\uline{Programmtechnisch identisch}} sind.
Die Gilden-Lehrer, so scheint ist, sollten eigentlich nur Mitglieder ihrer jeweiligen Gilde oder Fraktion ausbilden, machen aber -- das ist an dieser Stelle nachträglich mit Bleistift notiert -- auf niedriger Stufe (\enquote{kl. Lev.}) \enquote{entgegen dem Willen ihrer Gilde} auch Ausnahmen.\autocite[S.~15]{orpheus_b_scribbles}

Einen Einblick in die geplanten Fähigkeiten gewinnen wir unter anderem durch die Eintragungen in \enquote{\textsc{Orpheus} -- Spielwelt/Gildensystem}, wo für viele \enquote{\textsc{\uline{Gilde[n]}}} ein \enquote{\textsc{\uline{Lernprogramm}}} vermerkt ist.
Im Folgenden gruppiere ich die über verschiedene Gilden verstreuten Fähigkeiten der Übersichtlichkeit halber.
...
Während \enquote{Telekinetic Storm} erst mit dem 2015 für \textit{Pathfinder} erschienenen Quellenbuch \textit{Occult Adventures} als populärer Zauberspruch in Erscheinung zu treten scheint,\autocite[Vgl.][S.~188]{bulmahn_pathfinder_2015} kennen wir \enquote{Levitation} ebenfalls aus \textit{Advanced Dungeons \& Dragons} (hier als \enquote{Levitate} beziehungsweise \enquote{Schweben}).\footnote{Vgl. \autocite[S.~143]{cook_adnd_1989}, sowie \autocite[S.~186]{cook_adnd_1996}, dort als \enquote{Schweben}.} % Mike fragen, woher Telekinetic Storm stammt
An einer Stelle wird auf die \enquote{$\rightarrow$ Spruchtabelle!}\autocite[S.~10]{orpheus_gildensystem} verweisen, in der einzelne Zaubersprüche gesammelt sind (siehe \autoref{sec:orpheus_mechanik_magie}).

Daneben sind Schurken mit zahlreichen Fähigkeiten bedacht.
Wir finden Klassiker wie \enquote{Stealth},\autocite[S.~5]{orpheus_gildensystem} \enquote{Hide in Shadows}\autocite[S.~5]{orpheus_gildensystem}, \enquote{Pickpocket},\autocite[S.~9]{orpheus_gildensystem} \enquote{Schlösser öffnen},\autocite[S.~9]{orpheus_gildensystem} \enquote{Backstab}\footnote{\autocite[S.~5]{orpheus_gildensystem}. Als Steigerungsform finden wir auch einen \enquote{Major Backstab} (\autocite[S.~1]{orpheus_gildensystem}).} oder \enquote{Fallen erkennen}, \enquote{Fallen entschärfen}\autocite[S.~9]{orpheus_gildensystem} und \enquote{\sout{alle Fallen} manche Fallen [stellen]},\footnote{\autocite[S.~13]{orpheus_gildensystem}. Siehe auch \autocite[S.~15]{orpheus_gildensystem}.} die uns ebenfalls aus \textit{Advanced Dungeons \& Dragons} bekannt sind (\enquote{Stealth} dort als \enquote{Move Silently} beziehungsweise \enquote{Leise bewegen}).\footnote{Vgl. \autocite[S.~39\,f., 63]{cook_adnd_1989}, sowie \autocite[S.~56\,ff., 80]{cook_adnd_1996}.}
% Hinzu kommen

% Daneben gibt es noch zwei weitere Kampfbezogene Fähigkeiten, die sich unter Umständen hier einordnen lassen:
% \enquote{Critical Hit} (\autocite[S.~1]{orpheus_gildensystem})
% \enquote{Waffe vergiften \fbox{??}}\autocite[S.~8]{orpheus_b_scribbles}

% \enquote{Ausweichen}\autocite[S.~5]{orpheus_gildensystem}
% \enquote{Verkleiden}\footnote{\autocite[S.~5]{orpheus_gildensystem}. An anderer Stelle ist die Idee notiert: \enquote{Waffen heben \uline{Tarnung auf!}} (\autocite[S.~18]{orpheus_b_scribbles}).}
% \enquote{Lügen}\autocite[S.~5]{orpheus_gildensystem}

% \enquote{Akrobatik}
% \enquote{Akrobatischer Sprung}\autocite[S.~9]{orpheus_gildensystem}
% \enquote{klettern}\autocite[S.~9]{orpheus_gildensystem}
% \enquote{Gänge freilegen}
% \enquote{\uline{eingestürzte} Gänge freilegen}
% \enquote{Minecrawlersecret (Benutze Behälter auf Minecrawler)}\autocite[S.~6]{orpheus_b_scribbles}
% \enquote{Schwimmen, Aktrobatik}\autocite[S.~14]{orpheus_b_scribbles}
% \enquote{Fallen}\autocite[S.~14]{orpheus_b_scribbles}

Kampffähigkeiten werden an dieser Stelle nicht weiter ausdifferenziert.
Es ist nur pauschal die Rede von \enquote{\uline{alle[n]} Kampftechniken}\autocite[S.~8]{orpheus_gildensystem} sowie von \enquote{Fernkampf}.\autocite[S.~13, 15]{orpheus_gildensystem}
Es gibt allerdings einen Hinweis, dass sich verschiedene Fähigkeiten auch in den Kampfanimationen widerspiegeln sollten:
\enquote{Waffentalente $\rightarrow$ Anims}.\autocite[S.~7]{orpheus_b_scribbles}
In den Notizen zur Kampfsteuerung (siehe \autoref{sec:orpheus_mechanik_steuerung}) erfahren wir, dass


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ORPHEUS – SPIELMECHANIK – MAGIE %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Magie}\label{sec:orpheus_mechanik_magie}
Schon in dieser frühen Konzeptionsphase zeigt sich die besondere Rolle des Erzes, beispielsweise in den Notizen zu den einzelnen Fraktionen (siehe \autoref{sec:orpheus_welt_fraktionen}) oder zum Spielablauf (siehe \autoref{sec:orpheus_geschichte_spielablauf}).
