...
Um dieses Zentrum herum finden wir, geschützt durch eine \enquote{äußere Befestigung}, ein Viertel für die \enquote{Diebe}, eines für die \enquote{Söldner}, mit einer \enquote{Arena} in seiner Mitte, sowie ein \enquote{08/15}-Viertel.
...
Der Südwesten der Karte wird vom \enquote{Söldnerlager} dominiert, um das herum Felder angedeutet sind; eine Idee, die es später in Form der Reisfelder des Neuen Lagers in das Spiel schaffen wird.
...
Aus den noblen Söldnern werden die \enquote{Paladine} und statt der freien Diebe finden wir nun die \enquote{Fallensteller}.
...
\begin{quote}
   ...
   \begin{itemize}
      ...
      \item[--] soz. Funktion
      \item[--] Lernprogramm!
      \item[--] Ausbruchsplan\footnote{Die eckigen Klammern zwischen dem dritten und vierten Spiegelstrich sind Bestandteil des Zitats; vermutlich wurde dieser Punkt im Nachhinein ergänzt.}
   \end{itemize}
\end{quote}

Außerdem ist hier bereits festgehalten, dass es \enquote{\uline{4 Lager}} geben soll -- nämlich \enquote{Das alte Lager}, \enquote{Das neue Lager}, die \enquote{\textsc{Psi}-Sekte} sowie das \enquote{Buddlerfort} -- und welche Gruppierungen in ihnen jeweils vorgesehen sind.
Wir erfahren, dass die \enquote{Buddler des alten Lagers} sich in eine \enquote{Freizeitschicht} und eine \enquote{Arbeitsschicht} aufteilen, dass neben den vier Lagern noch ein \enquote{Böser Beschwörer} beziehungsweise \enquote{Dämonenbeschwörer} eine Rolle spielt und dass es im Sektenlager eventuell auch \enquote{Sekten-Jünger (08/15)} und \enquote{Sekten Groupies (\textsc{keine Gilde})} geben sollte.
In dieser Auflistung sind die \enquote{Sekten-Jünger} sowie die dem \enquote{Buddlerfort} beigerechneten \enquote{Halborks} außerdem von späterer Hand in eckige Klammern gesetzt worden; im gleichen Zuge wurden dafür wahrscheinlich \enquote{Tempelwache}, \enquote{Sekten Groupies} sowie \enquote{Freie Diebe} ergänzt.
Ebenso von späterer Hand stammt die Zuordnung einiger Gilden zu klassischen Archetypen.
Im Alten Lager ist neben der \enquote{Mafia} in Klammern \enquote{Söldner} und \enquote{Ritter} notiert, neben den \enquote{Feuermagiern} steht, relativ trivial, \enquote{Magier}, neben den \enquote{Schläger[n]} ist es \enquote{Barbar} und neben den \enquote{Spitzel[n]} wiederum \enquote{Assassin}.
...
Eine dritte Klasse, die \enquote{\uline{\sout{Mini Gilden}}}, die von \enquote{\sout{Lev[el] 1 -- 3}} hätten reichen sollen (\enquote{\sout{Alte Buddler}, \sout{Freie Buddler}}), ist von späterer Hand wieder durchgestrichen worden.
...
\enquote{Att (minus) bei Frage\uline{orgie}} ist hier vermerkt, wobei nicht ganz klar ist, wofür \enquote{Att} steht; eventuell ist damit Attitüde gemeint, vielleicht aber auch Attribut. % Mike Hoge fragen.
